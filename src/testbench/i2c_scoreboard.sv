...
class i2c_scoreboard extends uvm_scoreboard; // Defines the scoreboard class, inheriting from the UVM base scoreboard.
    ...
    virtual function void write(i2c_seq_item item);
        ...
        if (!expected_fifo.try_get(exp_item)) 
            begin
                // `uvm_error("SCOREBOARD", "Received transaction from Monitor but Expected FIFO is empty")
                return;
            end

        // Compare Address
        if (item.addr !== exp_item.addr) 
            begin
                `uvm_error("SCOREBOARD", $sformatf("Address Mismatch! Exp: 0x%0h, Act: 0x%0h", exp_item.addr, item.addr))
            end
    endfunction
